...
of complete published lexicons — public domain or openly
licensed, presented as published — that load on demand and are then
available offline:
...
There is no open full dictionary for Hittite or Old South Arabian, so those
languages are covered only by the curated comparative entries. The data
files live in `public/dicts/` and are generated by the `scripts/import-*.mjs`
importers, each of which documents its source and license.

The third Dictionary mode, **By meaning**, searches all six published
dictionaries together through their shared English glosses. English input
...
- Every cuneiform and hieroglyphic character in the database was generated
  from its verified Unicode codepoint (never typed from memory), and the
  smoke test pins each script's strings inside its own Unicode block.
- The comparative entries are hand-curated, never bulk-imported; the About
  ...
  TLA/Wb., CAL/TAD, DASI/CSAI, BDB/Strong's). The one imported dataset —
  Strong's Hebrew lexicon — is public domain, unmodified, and labeled as a
  published reference work (machine-readable arrangement by Open
  Scriptures, CC-BY-SA).
